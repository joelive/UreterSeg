...
from .nnFormer_net import SegmentationNetwork
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
        ...
        input_size=[]    
        with torch.no_grad():
            x=torch.zeros([2, 1, 128, 256, 224])
            for i in range(0, 4):
                x = self.downsample_layers[i](x)   
                input_size.append(x.shape[2]*x.shape[3]*x.shape[4])
                print(x.shape[2]*x.shape[3]*x.shape[4])       
